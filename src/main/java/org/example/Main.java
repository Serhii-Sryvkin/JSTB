...
public class Main extends TelegramLongPollingBot {
    ...
    public String getBotToken() {
        return "5875636714:AAHWPV2qMVHOkBBGpfWEQFMzKo8YDpFsp0c";
    }
    ...
    public void onUpdateReceived(Update update) {
        ...
        if (update.hasCallbackQuery()) {
            if (update.getCallbackQuery().getData().equals("btn_1") && getLevel(chatId) == 1) {
                setLevel(chatId, 2);
        ...
        if (update.hasCallbackQuery()) {
            if (update.getCallbackQuery().getData().equals("btn_2") && getLevel(chatId) == 2) {
                setLevel(chatId,3);
        ...
        if (update.hasCallbackQuery()) {
            if (update.getCallbackQuery().getData().equals("btn_3") && getLevel(chatId) == 3) {
                setLevel(chatId,4);
                ...
                message.setChatId(chatId);
                attachButtons(message, Map.of("Купити Джавелін (50 монет)", "btn_4"));
                sendApiMethodAsync(message);
            }
        }
        if (update.hasCallbackQuery()) {
            if (update.getCallbackQuery().getData().equals("btn_4") && getLevel(chatId) == 4) {
                setLevel(chatId,5);
    ...
    public int getLevel(Long chatId) {return levels.getOrDefault(chatId, 1);}
    public void setLevel (Long chatId, int level) {levels.put(chatId, level);}
